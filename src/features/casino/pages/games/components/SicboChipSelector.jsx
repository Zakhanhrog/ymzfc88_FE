...
const formatChipLabel = (label) => (label.endsWith('K') ? label.replace(/K$/, '') : label);

const SicboChipSelector = ({
  chipOptions = defaultSicboChipOptions,
  selectedChipValue,
  onSelectChip,
  ...
  useEffect(() => {
    ...
    window.addEventListener(SICBO_CUSTOM_CHIP_EVENT, handleCustomChipUpdate);
    return () => {
      window.removeEventListener(SICBO_CUSTOM_CHIP_EVENT, handleCustomChipUpdate);
    };
  }, []);
  ...
  const safeOnSelectChip = useCallback(
    (value) => {
      if (typeof onSelectChip === 'function') {
        onSelectChip(value);
      }
    },
    [onSelectChip]
  ...
  const safeOnOpenCustomChipModal = useCallback(() => {
    if (typeof onOpenCustomChipModal === 'function') {
      onOpenCustomChipModal();
    }
  }, [onOpenCustomChipModal]);

  const safeOnClearCustomChip = useCallback(
    (chipInfo) => {
      if (typeof onClearCustomChip === 'function') {
  ...
  const availableChips = useMemo(() => {
    if (!Array.isArray(chipOptions) || chipOptions.length === 0) {
      return defaultSicboChipOptions;
    }
    return chipOptions;
  }, [chipOptions]);
  ...
  const renderChipButton = (chip) => {
    ...
    return (
      <button
        key={chip.value}
        type="button"
        onClick={() => safeOnSelectChip(chip.value)}
        className={`relative flex h-11 w-11 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-full transition bg-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/60 ${
          isSelected ? 'opacity-100 shadow-lg shadow-emerald-600/20' : 'opacity-60 hover:opacity-95 hover:shadow-md'
        }`}
  ...
  return (
    <div className="grid items-center gap-1 rounded-2xl p-0.5 sm:p-1" style={{ gridTemplateColumns: 'minmax(0,6fr) minmax(0,1fr)' }}>
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-0.5 sm:gap-1">
        <button
          type="button"
      ...
      <div className="flex items-center justify-center pb-0.5">
        <div className="relative flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center">
          {customChip.value != null ? (
            <button
              type="button"
              onClick={() => safeOnSelectChip(customChip.value)}
              ...
              <span
                ...
                onClick={(event) => {
                  ...
                  if (selectedChipValue === customChip.value) {
                    safeOnSelectChip(null);
                  }
                  safeOnClearCustomChip(chipInfo);
                }}
                onKeyDown={(event) => {
...
export default SicboChipSelector;
