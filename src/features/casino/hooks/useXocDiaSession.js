...
const useXocDiaSession = ({ pollIntervalMs = 4000 } = {}) => {
  ...
  const fetchCurrentSession = useCallback(async () => {
    try {
      const response = await xocDiaSessionService.getCurrentSession();
      if (response?.success && response.data) {
        ...
        if (normalizedStatus !== 'RUNNING') {
          ...
          phaseStartedAtRef.current = null;
          phaseCompletionRef.current = null;
          setResultCode((prevCode) =>
            normalizedStatus === 'IDLE' ? null : prevCode
          );
          setTimerState((prev) => {
            if (
              prev.phaseKey === DEFAULT_TIMER_STATE.phaseKey &&
        ...
        const countdownAngle =
          isCountdown && remaining != null && duration
            ? clamp((remaining / duration) * 360, 0, 360)
            : 0;
...
export default useXocDiaSession;
